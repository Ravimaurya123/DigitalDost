...
const suggestions = [
  ...
  },
  {
    ...
    prompt:
      "Create a practical study plan for my DSA preparation",
    icon: "📚",
  },
  {
    ...
    prompt:
      "Help me understand Java programming with simple examples",
    icon: "☕",
  },
  {
    ...
    prompt:
      "Give me a roadmap to become a software developer",
    icon: "🚀",
  },
];
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={clearChat}
              className="hidden rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-300 transition hover:bg-white/10 hover:text-white sm:block"
            >
              Clear Chat
            </button>
      ...
      <section className="mx-auto flex min-h-[calc(100vh-73px)] max-w-7xl flex-col px-4 py-6 sm:px-6 lg:py-8">
        {/* HERO */}
        <div className="mb-6">
          <div className="mb-3 inline-flex items-center gap-2 rounded-full border border-cyan-400/20 bg-cyan-400/10 px-3 py-1 text-xs font-medium text-cyan-400">
            <span>✦</span>
            DIGITALDOST AI
          </div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400 sm:text-base">
            ...
            solve problems and get help with your daily
            activities.
          </p>
        </div>

        {/* CHAT CARD */}
        <div className="flex min-h-[560px] flex-1 flex-col overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] shadow-2xl shadow-black/20">
          {/* CHAT HEADER */}
          <div className="flex items-center justify-between border-b border-white/10 bg-white/[0.03] px-4 py-4 sm:px-6">
            ...
            <button
              type="button"
              onClick={clearChat}
              className="rounded-lg px-3 py-2 text-xs text-slate-500 transition hover:bg-white/5 hover:text-white sm:hidden"
            >
              Clear
            </button>
          </div>
          ...
          <div className="border-t border-white/10 bg-slate-950/60 p-3 sm:p-4">
            ...
            >
              <input
                ...
                disabled={loading}
                className="min-w-0 flex-1 rounded-xl border border-white/10 bg-white/[0.04] px-4 py-3 text-sm text-white outline-none transition placeholder:text-slate-600 focus:border-cyan-400/50 focus:bg-white/[0.06] disabled:opacity-50 sm:rounded-2xl sm:px-5 sm:py-4"
              />

              <button
                type="submit"
                disabled={loading || !input.trim()}
                className="rounded-xl bg-cyan-500 px-4 py-3 font-semibold text-slate-950 transition hover:bg-cyan-400 disabled:cursor-not-allowed disabled:opacity-40 sm:rounded-2xl sm:px-6"
              >
            ...
            <p className="mx-auto mt-2 max-w-4xl text-center text-[11px] text-slate-600">
              DigitalDost AI can make mistakes. Verify important
              information.
            </p>
          </div>
        </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {suggestions.map((item) => (
              <button
                ...
                className="group rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left transition hover:-translate-y-0.5 hover:border-cyan-400/20 hover:bg-white/[0.06]"
              >
                <div className="mb-3 flex h-9 w-9 items-center justify-center rounded-xl bg-cyan-400/10 text-lg transition group-hover:bg-cyan-400/20">
                  {item.icon}
                </div>
